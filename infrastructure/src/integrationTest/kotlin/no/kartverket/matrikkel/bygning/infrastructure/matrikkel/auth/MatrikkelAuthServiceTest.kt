...
class MatrikkelAuthServiceTest {
    @Test
    fun invalidUser() = runTest {
        wireMock.stubFor(WireMock.post("/matrikkelapi/wsapi/v1/BrukerServiceWS").willReturn(WireMock.unauthorized()))
    ...
    @Test
    fun innsynHeleLandet() = runTest {
        wireMock.stubFor(
            WireMock.post("/matrikkelapi/wsapi/v1/BrukerServiceWS")
                .withHeader(
                    "SOAPAction",
                    equalTo("\"http://matrikkel.statkart.no/matrikkelapi/wsapi/v1/service/bruker/BrukerService/getPaloggetBrukerRequest\""),
                )
                .willReturn(
        ...
        wireMock.stubFor(
            WireMock.post("/matrikkelapi/wsapi/v1/BrukerServiceWS")
                .withHeader(
                    "SOAPAction",
                    equalTo("\"http://matrikkel.statkart.no/matrikkelapi/wsapi/v1/service/bruker/BrukerService/findBrukerRettigheterForPaaloggetBrukerRequest\""),
                )
                .willReturn(
                    okForContentTypeFile(
        ...
        wireMock.stubFor(
            WireMock.post("/matrikkelapi/wsapi/v1/StoreServiceWS")
                .withRequestBody(
                    matchingXPath("//soapenv:Envelope/soapenv:Body/stor:getObjects/stor:ids[dom:item[dom:value[text()='2']] and dom:item[dom:value[text()='3']]]")
                        .withXPathNamespace("soapenv", "http://schemas.xmlsoap.org/soap/envelope/")
                        .withXPathNamespace("stor", "http://matrikkel.statkart.no/matrikkelapi/wsapi/v1/service/store")
    ...
    @Test
    fun berettigetInteresse_tidligereInnsyn() = runTest {
        wireMock.stubFor(
            WireMock.post("/matrikkelapi/wsapi/v1/BrukerServiceWS")
                .withHeader(
                    "SOAPAction",
        ...
        assertThat(matrikkelAuthService.harMatrikkeltilgang("random", Matrikkelrolle.InnsynMedPersondata)).isNull()
        assertThat(matrikkelAuthService.harMatrikkeltilgang("random", Matrikkelrolle.BerettigetInteresse)).isEqualTo("testbruker")

        wireMock.verify(4, postRequestedFor(urlEqualTo("/matrikkelapi/wsapi/v1/BrukerServiceWS")).withHeader("Authorization", equalTo("Bearer random")))
        wireMock.verify(2, postRequestedFor(urlEqualTo("/matrikkelapi/wsapi/v1/StoreServiceWS")).withHeader("Authorization", equalTo("Bearer random")))
    }

    private fun okForContentTypeFile(fileName: String): ResponseDefinitionBuilder {
        return WireMock.aResponse().withStatus(200).withHeader(ContentTypes.CONTENT_TYPE, *arrayOf("text/xml")).withBodyFile(fileName)
    }
