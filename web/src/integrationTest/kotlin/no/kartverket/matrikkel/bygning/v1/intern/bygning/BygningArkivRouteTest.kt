...
class BygningArkivRouteTest : TestApplicationWithDb() {
    @Test
    fun `gitt manglende entra jwt token svarer arkiv med 401`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()

            val response =
                client.get("/v1/intern/arkiv/bygninger/1/egenregistrert?registreringstidspunkt=${Instant.now()}")
            assertThat(response.status).isEqualTo(HttpStatusCode.Unauthorized)
        }

    @Test
    fun `gitt entra jwt token med ingen roller svarer arkiv med 401`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()
    ...
    @Test
    fun `gitt entra jwt token med feil rolle svarer arkiv med 401`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()
    ...
    @Test
    fun `gitt entra jwt token med feil audience svarer arkiv med 401`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()
    ...
    @Test
    fun `gitt et ugyldig dato query parameter svarer bygning arkiv bad request`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()

            val entraIdToken = mockOAuthServer.issueM2MEntraJwt()
            val response =
                client.get("/v1/intern/arkiv/bygninger/1/egenregistrert?registreringstidspunkt=UGYLDIG_DATO") {
                    bearerAuth(entraIdToken.serialize())
                }
    ...
    @Test
    fun `gitt et gyldig dato query parameter svarer bygning arkiv ok`() =
        testApplication {
            val client = mainModuleWithDatabaseEnvironmentAndClient()
            ...
            val oldData =
                client.get("/v1/intern/arkiv/bygninger/1/egenregistrert?registreringstidspunkt=2025-01-01T15:15:47.361080Z") {
                    bearerAuth(entraIdToken.serialize())
                }
