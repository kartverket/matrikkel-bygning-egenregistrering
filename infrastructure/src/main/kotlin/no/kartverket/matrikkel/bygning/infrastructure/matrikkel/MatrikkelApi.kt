...
class MatrikkelApi(
    ...
    val matrikkelContext =
        MatrikkelContext().apply {
            systemVersion = "4.18"
    ...
    fun withAuth(
        username: String,
        password: String,
    ): WithAuth = WithAuthImpl(BasicAuthAuthenticator(username, password))

    fun withAuth(bearerToken: String): WithAuth = WithAuthImpl(BearerTokenAuthenticator(bearerToken))
    ...
    private inner class WithAuthImpl(
        ...
        private fun BindingProvider.configure(endpointPath: String) {
            ...
            MDC.get("request_id")?.let {
                (
                    requestContext.computeIfAbsent(MessageContext.HTTP_REQUEST_HEADERS) {
                        HashMap<String, MutableList<String>>()
                    } as HashMap<String, MutableList<String>>
    ...
    private class BasicAuthAuthenticator(
    ...
    private class BearerTokenAuthenticator(
        private val bearerToken: String,
    ) : Authenticator {
        override fun invoke(requestContext: MutableMap<String, Any?>) {
            requestContext[MessageContext.HTTP_REQUEST_HEADERS] =
                mapOf(
