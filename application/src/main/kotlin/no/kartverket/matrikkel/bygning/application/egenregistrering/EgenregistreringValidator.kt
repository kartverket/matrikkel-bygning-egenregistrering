    ...
    companion object {
        ...
        private fun validateBruksenheterRegistreredOnCorrectBygning(
            ...
            if (invalidBruksenheter.isNotEmpty()) {
                return ValidationError("Bruksenhet${if (invalidBruksenheter.size > 1) "er" else ""} med ID ${invalidBruksenheter.joinToString()} finnes ikke i bygning med ID ${bygning.bygningId}")
            }
        ...
        private fun validateRepeatedBruksenheter(egenregistrering: Egenregistrering): ValidationError? {
            ...
            if (repeatedBruksenheter.isNotEmpty()) {
                return ValidationError("Bruksenhet ${if (repeatedBruksenheter.size > 1) "er" else ""} med ID ${repeatedBruksenheter.joinToString()} har flere registreringer på seg. Kun én registrering per bruksenhet kan sendes inn")
            }
